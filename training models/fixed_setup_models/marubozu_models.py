...
base_path = f"ml datasets/{symbol}/marubozu"
scaler_path = "scalers"
...
for filename in os.listdir(base_path):
    ...
    if "target" not in df.columns or "candle_size" not in df.columns:
        continue
    ...
    # Load scaler
    scaler_name = f"scaler_{symbol}_marubozu_{direction}_{timeframe}.pkl"
    ...
    try:
        gap_index = list(scaler.feature_names_in_).index("candle_size")
        gap_min = scaler.data_min_[gap_index]
        gap_max = scaler.data_max_[gap_index]
        gap_scaled = df["candle_size"].values
