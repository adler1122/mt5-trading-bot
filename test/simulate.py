...
def execute_trade(signal, entry_price, balance, ledger, balance_history, future_candles):
    ...
    if signal["order_type"] == "buy" and not signal["sl"] <= entry_price <= signal["tp"]: return balance
    if signal["order_type"] == "sell" and not signal["tp"] <= entry_price <= signal["sl"]: return balance
    result = simulate_tp_sl(signal, future_candles)
    if result=="open" or result=="invalid": return balance
