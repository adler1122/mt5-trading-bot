...
for tf in timeframes:
    ...
    for i in range(len(df) - 3):
        ...
        if (
            ...
            c2["open"] > c1["low"] and c2["close"] < c1["high"] and
        ...
        # Three Inside Down (bearish reversal)
            ...
            c1["close"] > c1["open"] and
            c2["close"] < c2["open"] and
            c2["open"] < c1["high"] and c2["close"] > c1["low"] and
            c3["close"] < c3["open"] and
            c3["close"] < c2["low"]
        ):
            break_price = c3["close"]
    ...
    pd.DataFrame(bullish_records).to_csv(f"bullish_threeinside_{symbol}_{tf}.csv", index=False)
    pd.DataFrame(bearish_records).to_csv(f"bearish_threeinside_{symbol}_{tf}.csv", index=False)