...
for tf in timeframes:
    ...
    for i in range(len(df) - 2):
        ...
        # Bullish Tweezer Bottom: two candles with equal lows, second candle bullish
        if (
            abs(c1["low"] - c2["low"]) < 0.05 and
        ...
        # Bearish Tweezer Top: two candles with equal highs, second candle bearish
        elif (
            abs(c1["high"] - c2["high"]) < 0.05 and
            c1["close"] > c1["open"] and
            c2["close"] < c2["open"]
        ):
            break_price = c2["close"]
    ...
    pd.DataFrame(bullish_records).to_csv(f"bullish_tweezer_{symbol}_{tf}.csv", index=False)
    pd.DataFrame(bearish_records).to_csv(f"bearish_tweezer_{symbol}_{tf}.csv", index=False)