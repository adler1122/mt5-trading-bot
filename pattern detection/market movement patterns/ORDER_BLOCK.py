...
for tf in timeframes:
    ...
    for i in range(len(df) - 2):
        ...
        # Softened Bullish Order Block
        if (
            c1["close"] < c1["open"] and (
                c2["close"] < c2["open"] or
                (c2["close"] > c2["open"] and c2["open"] < c1["close"])
            )
        ):
    ...
    pd.DataFrame(bullish_records).to_csv(f"bullish_orderblock_{symbol}_{tf}.csv", index=False)
    pd.DataFrame(bearish_records).to_csv(f"bearish_orderblock_{symbol}_{tf}.csv", index=False)